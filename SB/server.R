...
load(file='/home/ben/ShinyApps/pitAntenna/SB/cdForShiny.RData') # cdSB
print(c("after load cdForShiny", Sys.time() - startTime))

load(file='/home/ben/ShinyApps/pitAntenna/SB/tidePred.RData') #tidePred
print(c("after load tide",Sys.time() - startTime)) 

load(file='/home/ben/ShinyApps/pitAntenna/SB/envDataSBForMM.RData') #envDataSB
print(c("after load env",Sys.time() - startTime)) 

load(file='/home/ben/ShinyApps/pitAntenna/SB/sampDateRange.RData') #envDataSB
print(c("after load sampDateRange",Sys.time() - startTime)) 

load(file='/home/ben/ShinyApps/pitAntenna/SB/countsByDay.RData') #countsByDay, portableSampleDays
print(c("after load countsByDay",Sys.time() - startTime))

palette1 <- (c("#EA602E", "#4DAF4A", "#377EB8","#E41A1C",  "#984EA3",
          "#FF7F00", "#FFFF33", "#A65628", "#F781BF", "#999999"))

stationaryRM <- cdSB %>% filter(survey == "stationaryAntenna") %>% distinct(riverMeter) %>% filter(!is.na(riverMeter))
acousticRM <- cdSB %>% filter(survey == "acoustic") %>% distinct(riverMeter) %>% filter(!is.na(riverMeter))

shinyServer(function(input, output, session) {
  
  # filter for the selected ID
  selectedData <- reactive({
    cdSB %>% filter(tag == input$tag)
  })
  ...
  observeEvent(input$tag, {
   ...
   selectedDataTide <- reactive({
     tidePred %>% filter( DateTime >= xMin() & DateTime <= xMax() )
   })  
   
   selectedDataEnv <- reactive({
     envDataSB %>% filter( date >= xMin() & date <= xMax() )
   }) 
...
print(c("in RenderPlot",xMin(),xMax()))
    ...
    p <- selectedData() %>% arrange(detectionDate) %>%
      ...
      scale_y_continuous("River meter from ocean") +
    ...
    if(input$sampDate) {
      ...
      p <- p + 
        geom_vline(data= sampDateRange,aes(xintercept = as.numeric(minDate)), color="lightgreen", size=0.5) +
        geom_vline(data= sampDateRange,aes(xintercept = as.numeric(maxDate)), color="red", size=0.5)
    }  
    ...
    if(input$tide) {
      print(c("in tide",xMin(),xMax(),ranges$x,ranges$y))
      p <- p + geom_line( aes(DateTime,tideAdj), color="red", data=selectedDataTide() ) 
    }
  ...
  output$hover_info <- renderUI({
    ...
    if (nrow(point) > 0) print(c("in point",point,nrow(point)))
