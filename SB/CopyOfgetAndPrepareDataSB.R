...
cdSB$sizeForGraph <- ifelse( is.na(cdSB$observedLength), 3.5, cdSB$observedLength/20 )
...
save(cdSB, file='/home/ben/git/movementModel/shinyApp/cdForShiny.RData')

counts <- counts %>% filter(n > 1)
save(counts, file='/home/ben/git/movementModel/shinyApp/counts.RData')
# for testing
#cdShort <- cdSB %>% 
#             filter(tag %in% c(  '257c683e2f', '00088d1c80', '00088d2e1f')) %>% 
#             select(tag,riverMeter,detectionDate,survey,sizeForGraph)
...
save(envDataSB,file='/home/ben/git/movementModel/shinyApp/envDataSBForMM.RData')
...
# takes a while to run - only run when need to change
 ...
 save(tidePred,file='/home/ben/git/movementModel/shinyApp/tidePred.RData')
